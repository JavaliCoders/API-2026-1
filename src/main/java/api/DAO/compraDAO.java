...
public class compraDAO {
        ...
        try (Connection con = ConexaoDB.getConexao()) {
            ...
            try {
                ...
                try (PreparedStatement ps = con.prepareStatement(
                        sqlCompra, Statement.RETURN_GENERATED_KEYS)) {
                ...
                try (PreparedStatement psItem = con.prepareStatement(sqlItem)) {
                    for (CompraItem item : itens) {
                        ...
                        double jaComprado = getQtdCompradaPorPedidoProduto(
                                pp.getIdPedidoProduto());
                        double pendente = pp.getQtdAprovada() - jaComprado;
                        if (item.getQtdComprada() > pendente) {
                            System.err.println("Qtd excede pendente para: "
                                    + pp.getNomeProduto());
                            con.rollback();
                            return -3;
                        }
    ...
    public static boolean cancelar(int idCompra) {
        String sqlCancelar =
                "UPDATE tb_compra SET status = 'CANCELADA' WHERE id_compra = ?";
        try (Connection con = ConexaoDB.getConexao()) {
            con.setAutoCommit(false);
            try {
        ...
        try (Connection con = ConexaoDB.getConexao();
            ...
            while (rs.next()) {
                ...
                lista.add(new Compra(
                        rs.getInt("id_compra"), pedido, f,
                        rs.getTimestamp("data").toLocalDateTime(), u,
                        rs.getDouble("valor_total"),
                        tsPrev != null ? tsPrev.toLocalDateTime() : null,
                        rs.getString("status")   // ← adicionar
                ));
            }
        ...
        try (Connection con = ConexaoDB.getConexao();
            ...
            while (rs.next()) {
                ...
                lista.add(new CompraItem(
                        rs.getInt("id_compra_item"), idCompra, pp,
    ...
    // ── LISTAR compras disponíveis para nova nota fiscal ──────────────────────
    // Aparece enquanto qtd recebida (em NFs não recusadas) < qtd comprada.
    // Permite múltiplas NFs parciais até recebimento total.
    // NF ainda REGISTRADA (pendente de conferência) não conta como recebida,
    // evitando NF duplicada enquanto outra ainda está em aberto.
    public static ObservableList<Compra> listarRealizadasSemNota() {
                ...
                WHERE c.status = 'REALIZADA'
                  AND EXISTS (
                      ...
                      WHERE ci.id_compra = c.id_compra
                        AND ci.qtd_comprada > COALESCE((
                            SELECT SUM(ni.qtd_recebida)
                            FROM tb_nf_item    ni
                            JOIN tb_notasfiscal nf ON nf.id_nota = ni.id_nota
                            WHERE nf.id_compra        = c.id_compra
                              AND nf.status IN ('CONFERIDA','DIVERGENTE')
                              AND ni.id_pedido_produto = ci.id_pedido_produto
                        ), 0)
                  )
        ...
        try (Connection con = ConexaoDB.getConexao();
            ...
            while (rs.next()) {
                ...
                lista.add(new Compra(
                        rs.getInt   ("id_compra"), pedido, f,
                        rs.getTimestamp("data").toLocalDateTime(), comprador,
                        rs.getDouble("valor_total"),
                        tsPrev != null ? tsPrev.toLocalDateTime() : null,
                        rs.getString("status")   // ← adicionar
                ));
    ...
    private static void atualizarStatusPedido(Connection con, int idPedido)
            throws SQLException {
        String sql = """
                UPDATE tb_pedido
                SET status = (
    ...
    private static int getPedidoByCompra(Connection con, int idCompra)
            throws SQLException {
