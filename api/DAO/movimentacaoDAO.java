...
public class movimentacaoDAO {
    ...
    public static boolean registrarSaida(int idPedido, int idProduto, int idPedidoProduto,
                                         int quantidade, int idUsuario) {
        String sqlSaldo = """
                UPDATE tb_produto
                SET saldo = saldo - ?
                WHERE id_produto = ?
        ...
        String sqlItem = """
                ...
                SET qtd_recebida = GREATEST(COALESCE(qtd_recebida, 0) - ?, 0)
        ...
        try (Connection con = ConexaoDB.getConexao()) {
            ...
            try (PreparedStatement psSaldo = con.prepareStatement(sqlSaldo);
                 PreparedStatement psMovimentacao = con.prepareStatement(sqlMovimentacao);
                 PreparedStatement psItem = con.prepareStatement(sqlItem)) {

                psSaldo.setInt(1, quantidade);
                psSaldo.setInt(2, idProduto);
                psSaldo.setInt(3, quantidade);
                if (psSaldo.executeUpdate() == 0) {
                    con.rollback();
                    return false;
                }

                psMovimentacao.setInt(1, idProduto);
                psMovimentacao.setInt(2, quantidade);
                psMovimentacao.setInt(3, idUsuario);
                psMovimentacao.setInt(4, idPedido);
                psMovimentacao.setString(5, "Saida do pedido " + idPedido + " - item " + idPedidoProduto);
                psMovimentacao.executeUpdate();

                psItem.setInt(1, quantidade);
                psItem.setInt(2, idPedidoProduto);
                ...
                con.rollback();
                System.err.println("Erro ao registrar saida: " + e.getMessage());
                return false;
            } finally {
